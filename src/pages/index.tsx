        ...
        <main id="site-main" className={`${SiteMain} ${outer}`}>
          <div className={`${inner}`}>
            <div className={`${PostFeed} ${PostFeedRaise}`}>
              {props.data.allMarkdownRemark.edges.filter(post => {
                return post.node.frontmatter.tags.includes("Projects");
              })
              .map(post => {
            ...
            {/* Blog section */}
            <h1 className="spacer">Blog</h1>

            <div className={`${PostFeed} ${PostFeedRaise}`}>
              {props.data.allMarkdownRemark.edges.filter(post => {
                return post.node.frontmatter.tags.includes("Blog-post");
              })
              .map(post => {
                return <PostCard key={post.node.fields.slug} post={post.node} />;
              }) 
...
export const pageQuery = graphql`
  query {
    ...
    allMarkdownRemark(
      ...
      # filter: { frontmatter: { tags: { in: ["Web"] } } }
